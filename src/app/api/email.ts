...
export const sendCustomerSupportEmail = async (
  emailData: CustomerSupportEmailRequestDTO
) => {
  const response = await apiClient.post(
    "/api/email/customer-support",
    emailData
  );
  return response.data;
};

export interface BulkCustomerSupportEmailRequestDTO {
  /** Trip ID — required */
  tripId: number;
  /** Email subject — required, max 200 characters */
  subject: string;
...
export const sendBulkCustomerSupportEmailForTrip = async (
  emailData: BulkCustomerSupportEmailRequestDTO
) => {
  const response = await apiClient.post(
    "/api/email/bulk/trip-customer-support",
    emailData
  );
  return response.data;
};
